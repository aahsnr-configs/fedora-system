An analysis of the provided `fedora-setup.py` script reveals a comprehensive tool for automating the setup of a Fedora system. The script is well-structured and includes many good practices like idempotency checks and colored logging. However, several areas can be improved for robustness, clarity, and modularity, especially when considering Python 3.13 and general maintainability.

The following rewrite addresses these points by introducing a more robust argument parsing system using `argparse`, improving command execution and error handling, and organizing the code into a more modular structure. This allows users to run specific parts of the setup independently.

### Key Improvements:

  * **Modular Execution with `argparse`**: The script now uses Python's `argparse` library to allow for modular execution. You can run the entire setup or choose specific tasks like `dnf`, `flatpak`, `nvidia`, or `dotfiles`. This makes the script more flexible for different use cases and easier to debug.
  * **Improved Command Handling**: The `run_command` function has been enhanced to be more robust. It now consistently handles `sudo` and user context switching, and provides clearer error messages.
  * **Python 3.13 Compatibility**: The script has been reviewed for compatibility with Python 3.13. Deprecated modules and functions have been avoided, ensuring future-proofing.
  * **Clarity and Readability**: The code has been reorganized into logical sections with clear headings. Docstrings and comments have been improved to make the script easier to understand and maintain.
  * **Error Checking and Idempotency**: The script's idempotency checks have been refined to be more reliable. Error handling is now more consistent across all functions.
  * **Single-File Simplicity**: As requested, the entire script is contained within a single file, making it easy to download and execute.

Below is the rewritten and improved `fedora-setup.py` script.
...
def run_command(
    ...
    **kwargs,
    ...
    try:
        ...
        log("ERROR", f"Command failed: {' '.join(e.cmd)} (Exit Code: {e.returncode})", Colors.RED)
        if e.stdout:
            print(f"--- STDOUT ---\n{e.stdout.strip()}")
        if e.stderr:
            print(f"--- STDERR ---\n{e.stderr.strip()}")
...
def pre_flight_checks():
    ...
    if not shutil.which("dnf"):
        log("ERROR", "DNF command not found. This script requires a DNF-based system.", Colors.RED)
        sys.exit(1)
...
def setup_dnf():
    ...
    if dnf_conf_src.is_file() and not filecmp.cmp(str(dnf_conf_src), str(dnf_conf_dest)):
...
def setup_core_packages():
    ...
    core_packages = [
        "git-core", "curl", "wget", "unzip", "p7zip", "make", "gcc", "gcc-c++",
        "kernel-devel", "kernel-headers", "akmod-nvidia", "xorg-x11-drv-nvidia-cuda",
        "vulkan-tools", "ffmpeg"
    ]
...
def setup_desktop_environment():
    ...
    de_packages = [
        "hyprland", "kitty", "rofi-wayland", "waybar", "swaybg",
        "xdg-desktop-portal-hyprland", "polkit-gnome", "thunar", "mako"
    ]
...
def setup_development_tools():
    ...
    dev_packages = [
        "neovim", "python3-neovim", "ripgrep", "fd-find", "stow",
        "nodejs", "npm", "golang", "rust", "cargo"
    ]
...
def configure_nvidia():
    ...
    if result.returncode != 0:
        log("WARNING", "akmod-nvidia is not installed. Skipping NVIDIA configuration.", Colors.YELLOW)
        return
    ...
    if not list(pathlib.Path("/lib/modules").glob(f"{current_kernel}/extra/nvidia*")):
        log("INFO", "NVIDIA kernel modules not found for the current kernel. Rebuilding...")
        run_command(["akmods", "--force", "--kernels", current_kernel], check=True)
        log("SUCCESS", "NVIDIA akmods rebuilt.", Colors.GREEN)
    else:
        log("INFO", "NVIDIA kernel modules already exist for the current kernel.", Colors.YELLOW)
...
def setup_flatpaks():
    ...
    flatpak_apps = [
        ...
        "md.obsidian.Obsidian", "com.visualstudio.code"
    ]
    for app in flatpak_apps:
        # Check if already installed
        result = run_command(["flatpak", "info", "--user", app], as_user=SUDO_USER, check=False)
        if result.returncode == 0:
            log("INFO", f"Flatpak app '{app}' is already installed.", Colors.YELLOW)
            continue
            
        log("INFO", f"Installing Flatpak app '{app}'...")
        run_command(["flatpak", "install", "--user", "-y", "flathub", app], as_user=SUDO_USER, check=True)
...
def setup_dotfiles():
    ...
    if not dotfiles_dir.is_dir():
        ...
        run_command(["git", "clone", dotfiles_repo_url, str(dotfiles_dir)], as_user=SUDO_USER, check=True)
    else:
    ...
    for pkg in stow_packages:
        ...
        run_command(["stow", "-R", pkg], cwd=dotfiles_dir, as_user=SUDO_USER, check=True)
...
""",
    ...
    log("INFO", f"Starting Fedora setup for tasks: {', '.join(tasks_to_run)}", Colors.BLUE)
    ...
    task_map = {
        ...
        "packages": lambda: [setup_core_packages(), setup_desktop_environment(), setup_development_tools()],
    ...
    for task_name in tasks_to_run:
        ...
        if task_func:
            try:
                ...
                log("FATAL", f"A critical error occurred during '{task_name}': {e}", Colors.RED)
                sys.exit(1)
        else:
